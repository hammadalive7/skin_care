...
class _BarColorPickerState extends State<BarColorPicker> {
  ...
  Widget build(BuildContext context) {
    ...
    late double thumbLeft, thumbTop = 0.0;
    if (horizontal) {
    ...
    final Widget frame = SizedBox(width: frameWidth, height: frameHeight);
    ...
    final Widget content = Positioned(
      left: left,
      top: top,
      child: Container(
        width: barWidth,
  ...
  void handleTouch(Offset globalPosition, BuildContext context) {
    ...
    switch (widget.pickMode) {
      case PickMode.color:
        final color = HSVColor.fromAHSV(1.0, percent * 360, 1.0, 1.0).toColor();
...
class _CircleColorPickerState extends State<CircleColorPicker> {
  ...
  void handleTouch(Offset globalPosition, BuildContext context) {
    ...
    final theta = atan2(deltaX, deltaY);
